...
bantalan} menggunakan dataset CWRU dengan pendekatan progresif dari keluarga
algoritma kernel (SVM/LR), tree (DT/RF/XGBoost), hingga jaringan dalam
(WDCNN). Setiap keluarga dilengkapi modul \emph{Explainable AI} (SHAP) yang
sesuai dengan arsitekturnya. Keluaran utama Jalur~A adalah
\emph{Fault Signature Maps} (FSM) — representasi \emph{fingerprint} per
kelas kerusakan pada resolusi sinyal mentah 2.048 titik.

Jalur~B berfokus pada \textbf{estimasi sisa umur pakai} (\emph{Remaining
Useful Life}, RUL) menggunakan tiga dataset prognostik (PHM2012, XJTU-SY,
IMS) dengan tiga arsitektur \emph{backbone}: Mamba-xLSTM-Net,
N-BEATS-xLSTM-RUL, dan SparseGate-TCN-RUL. Interpretabilitas Jalur~B
dicapai melalui \emph{Top-k Sparse Autoencoder} (SAE) yang memetakan
representasi laten model ke frekuensi karakteristik bantalan (BPFx) secara
...
\caption{Kerangka konseptual terintegrasi: Jalur~A (biru) menggunakan dataset
         CWRU untuk \emph{benchmark} lintas keluarga algoritma (kernel/tree/deep)
         dan menghasilkan FSM sebagai XAI level sinyal; Jalur~B (hijau)
...
Dataset CWRU merupakan sumber utama pada Bab~IV untuk \emph{benchmark}
lintas keluarga algoritma. Pada Bab~V, CWRU juga digunakan untuk validasi
\emph{cross-dataset} SAE; namun hanya tersedia $n = 10$ rekaman sehingga
daya statistik terbatas ($p > 0{,}05$) — kondisi ini ditandai secara
eksplisit pada §V.8.
...
tersegmentasi. Pemisahan tahap ketiga berdasarkan keluarga model.
...
jendela geser dengan ukuran yang disesuaikan per dataset dan keluarga model:
\begin{itemize}
...
\subsection{Health Indicator 36-D untuk Keluarga Kernel dan Tree}
\label{subsec:bab3_hi36d}

Keluarga kernel (SVM/LR) dan tree (DT/RF/XGBoost) pada Bab~IV tidak
beroperasi langsung pada sinyal mentah. Dari setiap segmen sinyal CWRU,
diekstraksi \textbf{vektor HI 36-D} yang terdiri atas 18 fitur per kanal
akselerometer (drive-end dan fan-end), masing-masing mencakup:
...
frekuensi $\varphi$, dan $d_{\rm lat} = 1.024$ adalah dimensi laten SAE.
Ambang batas 0,30 mengikuti konvensi korelasi moderat yang lazim dalam
literatur analisis sinyal.

\textbf{Bootstrap 95\% CI} dihitung dengan $B = 1.000$ resampel rekaman dari
...
tidak dapat dijelaskan oleh kebetulan semata dan merupakan properti emergent
dari representasi yang terlatih. Dua kelompok kontrol negatif juga digunakan:
...
atas tiga \emph{seed}: $\overline{M} \pm \sigma_M$.
Pada Jalur~A (\emph{benchmark} kernel/tree/deep), \emph{seed} ditetapkan
dari masing-masing notebook reproduksi karena pelatihan klasifikasi bersifat
deterministik dan stabil pada kondisi yang sama.

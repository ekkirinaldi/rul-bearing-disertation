...
\chapter{Estimasi Sisa Umur Pakai}
\label{bab:prognostik}

Bab ini menyajikan {\color{blue}komponen prognostik} dari penelitian ini secara \emph{end-to-end}, 
mulai dari perancangan metodologi tiga arsitektur \emph{backbone} RUL, pengembangan \emph{Top-$k$
Sparse Autoencoder} (SAE), prosedur pemetaan ke frekuensi karakteristik
\emph{bearing}, hingga inferensi statistik dan analisis hasil antar empat dataset.
Bab ini mendetailkan penelitian yang dipublikasikan pada \citetitb{TotoSuharto2025Journal2RUL}, yang
mengombinasikan tiga arsitektur berbeda sebagai {\color{blue}salah satu
penerapan awal} interpretabilitas mekanistik (\emph{mechanistic
interpretability}) {\color{blue}pada domain prognostik \emph{bearing}}.

Beberapa konsep yang menjadi landasan pembahasan bab ini, yaitu spesifikasi empat dataset
...
interpretabilitas SAE, serta infrastruktur komputasi, telah dijelaskan pada
Bab~\ref{bab:metodologi-umum}.
Secara sistematika penulisan, subbab §V.1 s.d.\ §V.4 membahas metodologi, sementara subbab §V.5 s.d.\ §V.9
...
Tiga arsitektur \emph{backbone} dipilih untuk mewakili tiga prinsip desain
yang berbeda dalam pemodelan deret waktu.
...
yang menargetkan pemodelan konteks panjang dengan presisi tinggi.
...
Variasi pada {\color{blue}bias induktif} ketiga arsitektur ini sengaja dipilih agar uji
universalitas interpretabilitas SAE-BPFx tidak bergantung pada satu
paradigma arsitektur tunggal.

Semua arsitektur yang dipilih mendapatkan input berupa sekuens vektor \emph{Health
Indicator} (HI) lima dimensi yang diekstraksi per rekaman.
{\color{blue}Sesuai definisi pada Bab~\ref{bab:metodologi-umum}~%
...
Mamba-xLSTM-Net adalah arsitektur hibrid yang menyatukan dua komponen
...
{\color{blue}\citetitb{Gu2023Mamba,Beck2024xLSTM}}.
...
dapat dilatih (\emph{gated fusion}), menghasilkan representasi laten
$\mathbf{h} \in \mathbb{R}^{d_{\rm model}}$ yang menjadi input MLP
dua-lapis untuk menghasilkan estimasi RUL.

Pada Bab~V, $d_{\rm model}$ dikonfigurasi berbeda per dataset untuk
menyeimbangkan kapasitas model dan risiko \emph{overfitting}, yaitu $d_{\rm model}
= 128$ pada PHM2012 yang memiliki sekitar 898.000 parameter total, dan
$d_{\rm model} = 112$ pada XJTU-SY yang menghasilkan sekitar 811.000 parameter.
Model yang dijalankan pada dataset IMS menggunakan konfigurasi yang sama dengan PHM2012 karena panjang sekuens
degradasi yang sebanding.
Detail lengkap hyperparameter arsitektur dapat dilihat pada
Lampiran~\ref{lamp:hparam}.

Mekanisme selektif arsitektur Mamba mampu mengevaluasi relevansi setiap titik dalam sekuens
...
untuk memprediksi RUL dengan {\color{blue}menempatkan sebuah
...
\emph{seed}. Karena \emph{prior} struktural basis-blok sudah memiliki
pengetahuan tentang bentuk kurva degradasi, maka ruang solusi yang perlu dijelajahi
...
SparseGate-TCN-RUL dibangun dari tumpukan {\color{blue}empat} blok konvolusi
kausal terdilatasi (\emph{Temporal Convolutional Network}, TCN) dengan laju
dilatasi yang meningkat secara eksponensial: {\color{blue}1, 2, 4, dan 8}.
...
{\color{blue}Sebelum tumpukan TCN, sebuah \emph{sparse feature gate}
...
oleh \emph{quantile head} yang menjaga keterurutan, dengan nilai median
dilaporkan sebagai estimasi RUL.

Arsitektur ini memiliki sekitar 249.000 parameter, sehingga paling ringan di antara
ketiga \emph{backbone}.
Kecepatan inferensi yang tinggi menjadikan SparseGate-TCN-RUL kandidat
yang sesuai untuk \emph{deployment} pada \emph{Edge Server}
...
kompetitif pada PHM2012 karena TCN terdilatasi secara alami sehingga dapat menangkap pola
...
pada NVIDIA~A40 {\color{blue}dengan tetap mempertahankan akurasi numerik setara komputasi 
\emph{floating point} 32-bit}.
...
\caption{Konfigurasi pelatihan bersama untuk ketiga \emph{backbone} RUL.
         Semua parameter berlaku pada dataset PHM2012, XJTU-SY, dan IMS.}
...
selama proses pelatihan, yang digunakan untuk evaluasi akhir pada
\emph{test set}.
...
{\color{blue}\citetitb{Bricken2023Monosemanticity,Cunningham2023TopkSAE}}
...
neuron \emph{deep network} konvensional yang bersifat \emph{polysemantik} (satu
...
Dengan definisi \emph{hidden state} backbone $\mathbf{h} \in \mathbb{R}^d$,
representasi laten SAE $\mathbf{z} \in \mathbb{R}^{d_{\rm lat}}$ dapat dihitung
sebagai:

\begin{equation}\label{eq:bab5_sae_topk}
  \mathbf{z} = \mathrm{TopK}\!\bigl({\color{blue}\mathrm{ReLU}}(\mathbf{W}_{\rm enc}
                (\mathbf{h} - \mathbf{b}_{\rm pre})),\; k\bigr),
\end{equation}

\noindent di mana $\mathrm{TopK}(\cdot, k)$ mempertahankan nilai terbesar
$k$ komponen {\color{blue}dari pra-aktivasi yang telah dilewatkan
...
{\color{blue}1,28 detik (XJTU-SY), dan 1,0 detik (IMS)}, mencakup puncak
...
\emph{hit-rate} yang teramati bukan merupakan artefak kebetulan, yaitu \emph{bootstrap
...
Tingkat kepercayaan 95\% diperoleh dari persentil ke-2,5 dan ke-97,5 distribusi
empiris $H_\varphi$ atas semua resampel, sebagaimana telah didefinisikan pada
Persamaan~\ref{eq:bab3_bootstrap_ci}.

\textbf{Permutation test{\color{blue} dua-sisi}.} Sebanyak $B = 1.000$
permutasi label rekaman dilakukan, dimana pada setiap permutasi, vektor amplitudo
$A_{r,\varphi}$ diubah acak terhadap indeks rekaman, memutus semua korelasi
temporal antara aktivasi SAE dan amplitudo BPFx.
Nilai $H_\varphi^{(b)}$ dihitung untuk setiap permutasi $b$; \emph{p-value}
{\color{blue}dua-sisi} diperoleh dengan
Persamaan~\ref{eq:bab3_permutation_pvalue} {\color{blue}sebagai proporsi
...
{\color{blue}Untuk mengendalikan beban pengujian ganda (\emph{multiple
testing}), satu BPFx dominan per pasangan (dataset, arsitektur) ditetapkan
sebagai luaran primer, yang menghasilkan 12 uji primer atas 4 dataset $\times$
...
{\color{blue}Analisis ini bertujuan menelusuri sensitivitas
...
pada salah satu \emph{bearing} dalam dataset PHM2012.
...
\begin{table}[ht]
\centering
\caption{Evaluasi kinerja pada dataset XJTU-SY tiga kondisi operasi yang
         lengkap (mean $\pm$ std, $n = 3$ \emph{seed}). Pemenang RMSE
         dicetak tebal.}
...
\textbf{Model} & \textbf{RMSE~$\downarrow$} & \textbf{MAE~$\downarrow$}
  & \textbf{$R^2$~$\uparrow$} & \textbf{PHM Score~$\uparrow$} \\
\midrule
\textbf{Mamba-xLSTM-Net}     & \textbf{0{,}213 $\pm$ 0{,}056}
  & 0{,}166 & 0{,}333 & \textbf{0{,}938} \\
SparseGate-TCN-RUL           & 0{,}216 $\pm$ 0{,}011 & \textbf{0{,}165}
  & \textbf{0{,}362} & 0{,}915 \\
N-BEATS-xLSTM-RUL            & 0{,}256 $\pm$ 0{,}010 & 0{,}218
  & 0{,}098 & 0{,}873 \\
\bottomrule
\end{tabular}
\end{table}

Pada dataset XJTU-SY tiga kondisi operasi yang lengkap, Mamba-xLSTM-Net
mencatat RMSE rata-rata terendah sebesar 0{,}213 sekaligus PHM~Score
tertinggi 0{,}938, menjadikannya pemenang pada dataset ini.
SparseGate-TCN-RUL menempati posisi kedua dengan selisih RMSE yang sangat
tipis (0{,}216), bahkan unggul pada MAE (0{,}165) dan $R^2$ (0{,}362),
sedangkan N-BEATS-xLSTM-RUL berada di posisi ketiga (RMSE 0{,}256).
Selisih RMSE dari terbaik ke terburuk adalah 0{,}043, lebih lebar
dibandingkan hasil pada subset dua kondisi sebelum perbaikan dataset karena
\emph{test split} tiga kondisi mencakup keragaman beban yang lebih besar.

Keunggulan RMSE Mamba-xLSTM-Net disertai variansi antar-\emph{seed} yang
relatif tinggi ($\sigma = \pm$0{,}056), terbesar di antara ketiga
\emph{backbone}; hal ini disebabkan oleh sebaran RMSE per \emph{seed}
yang lebar (0{,}153 pada \emph{seed}~42 hingga 0{,}289 pada \emph{seed}~44).
Sebaliknya, N-BEATS-xLSTM-RUL dan SparseGate-TCN-RUL menunjukkan stabilitas
lintas \emph{seed} yang lebih tinggi ($\sigma = \pm$0{,}010 dan
$\pm$0{,}011), mencerminkan \emph{prior} struktural yang menekan sensitivitas
terhadap inisialisasi acak. Nilai $R^2$ yang positif untuk ketiga model
menegaskan bahwa pada dataset tiga kondisi yang lengkap, prediksi RUL
seluruh \emph{backbone} mengungguli prediktor rata-rata, berbeda dari subset
dua \emph{bearing} sebelum perbaikan dataset.
...
\caption{Kurva prediksi RUL terhadap nilai referensi untuk \emph{Bearing}~1
         {\color{blue}pada \emph{Run}~1} dataset IMS (Mamba-xLSTM-Net,
...
XJTU-SY & Mamba-xLSTM-Net      & $0{,}213 \pm 0{,}056$ \\
...
Pertama, tidak ada satu arsitektur yang mendominasi semua dataset:
SparseGate-TCN-RUL unggul pada PHM2012, sedangkan Mamba-xLSTM-Net unggul
pada XJTU-SY dan IMS.
...
analisis SAE-BPFx.
Kedua, Mamba-xLSTM-Net memberikan kinerja prognostik terbaik pada dua dari
tiga dataset (RMSE 0{,}213 pada XJTU-SY dan 0{,}407 pada IMS), menjadikannya
pilihan \emph{backbone} utama untuk analisis interpretabilitas; meskipun pada
XJTU-SY keunggulan akurasinya disertai variansi antar-\emph{seed} yang lebih
tinggi dibandingkan kedua \emph{backbone} pembanding.
Ketiga, N-BEATS-xLSTM-RUL dan SparseGate-TCN-RUL memperlihatkan deviasi
standar terkecil pada XJTU-SY ($\sigma = \pm$0{,}010 dan $\pm$0{,}011),
mengkonfirmasi bahwa \emph{prior} struktural basis-blok degradasi dan gerbang
fitur \emph{sparse} menekan sensitivitas terhadap inisialisasi acak.
...
XJTU-SY & 1{,}5\% & 0{,}7\% & \textbf{1{,}6\%} & 0{,}2\% & $< 0{,}001$
  & BPFI/BSF/BPFO; keragaman mode kegagalan lintas kondisi \\
...
Pada XJTU-SY dengan dataset tiga kondisi operasi yang lengkap, \emph{hit-rate}
justru tersebar pada BSF (1{,}6\%), BPFI (1{,}5\%), dan BPFO (0{,}7\%), tidak
lagi terpusat pada satu frekuensi karakteristik. Sebaran ini mencerminkan
keragaman mode kegagalan lintas tiga kondisi beban pada \emph{bearing}
LDK~UER204 \citetitb{Wang2020XJTU}, berbeda dari subset dua kondisi sebelum
perbaikan dataset yang memperlihatkan dominasi \emph{outer-race} tunggal.

\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/bab5/hitrate_panel.pdf}
\caption{\emph{Hit-rate} fitur SAE ke BPFx (\emph{point estimate}) per
         dataset dengan anotasi nilai $p$ dari \emph{permutation test}.
         Batang berwarna biru untuk BPFI, merah untuk BPFO, abu-abu untuk
         BSF, dan ungu untuk FTF. Nilai \emph{hit-rate} konsisten dengan
         Tabel~\ref{tab:bab5_hitrate_utama}. Dataset CWRU ditandai sebagai
         \emph{underpowered} ($n = 10$, $p > 0{,}05$).
         \citetitb{TotoSuharto2025Journal2RUL}}
...
{\color{blue}Namun, IMS dilaporkan secara transparan sebagai
\emph{ketidakcocokan parsial} (\emph{partial mismatch}), dimana kegagalan yang
...
Oleh karena itu, dataset IMS merupakan satu-satunya dari empat dataset di mana BPFx
dominan tidak sepenuhnya selaras dengan moda kegagalan yang
terdokumentasi. Perbedaan ini dicatat untuk dapat dilakukan penelitianlanjutan.}
Implementasi pada dataset CWRU menunjukkan \emph{hit-rate} BPFI tertinggi secara numerik (5{,}08\%)
namun tidak mencapai signifikansi statistik ($p > 0{,}05$) karena hanya
tersedia 10 rekaman {\color{blue}pada tingkat berkas} untuk analisis
korelasi, menjadikannya \emph{underpowered}.
{\color{blue}Atas dasar inilah CWRU dikeluarkan dari klaim falsifikasi
formal penelitian (Novelti~N4) dan diperlakukan sebagai hasil rintisan
(\emph{pilot}) yang bersifat kualitatif.}

Korelasi Pearson maksimum yang teramati adalah $r_{\rm max} = 0{,}447$ pada
fitur SAE ke-443 terhadap BPFI untuk PHM2012, dan $r_{\rm max} = 0{,}468$
pada fitur ke-868 terhadap BPFO untuk XJTU-SY
\citetitb{TotoSuharto2025Journal2RUL}.
...
Fakta bahwa BPFI, BPFO, dan BSF muncul sebagai BPFx dengan \emph{hit-rate}
yang signifikan secara statistik menunjukkan korespondensi lemah namun
terdeteksi antara representasi yang dipelajari model dan teori getaran klasik.

\begin{figure}[ht]
\centering
\includegraphics[width=0.80\textwidth]{figures/bab5/corr_scatter_phm2012.pdf}
\caption{\emph{Scatter plot} aktivasi fitur SAE ke-443 terhadap amplitudo
         BPFI pada PHM2012 ($r = 0{,}447$, $n = 304$ rekaman \emph{training},
         4 bantalan tersedia dari 7 bantalan nominal).
...
aktivasi fitur SAE ke-443 dan amplitudo BPFI PHM2012.
...
Pola ini selaras dengan fisika degradasi, dimana amplitudo BPFI meningkat ketika
...
{\color{blue}Perlu dicatat bahwa panel CWRU pada uji kontrol negatif
berperilaku berbeda dan bersifat \emph{cautionary}: dengan hanya 10 rekaman
pada tingkat data, \emph{backbone} berinisialisasi acak (Xavier) bahkan
...
sampel dimana rancangan kontrol negatif tidak lagi informatif.
Oleh karena itu dataset CWRU dikeluarkan dari klaim falsifikasi formal dan diperlakukan
sebagai hasil rintisan kualitatif (lihat \S\ref{sec:bab5_hitrate}).}

Berdasarkan {\color{blue}hasil kedua kontrol pada tiga dataset yang
...
penjelasan alternatif {\color{blue}pada ketiga dataset tersebut}.
...
{\color{blue}Pada eksperimen lintas-arsitektur, sebuah SAE baru dilatih
secara terpisah pada \emph{hidden states} setiap \emph{backbone} dengananggaran
...
mandiri. Sebagai konsekuensi, magnituda absolut pada subbab ini dapat berbeda
dari Tabel~\ref{tab:bab5_hitrate_utama}.}

Pada PHM2012, Mamba-xLSTM-Net menghasilkan \emph{hit-rate} BPFI
tertinggi di antara ketiga arsitektur {\color{blue}(BPFI~3{,}42\%,
BPFO~3{,}03\%)}.
Hal ini konsisten dengan {\color{blue}bias induktif} SSM selektif yang secara alami
merespons pola temporal berfrekuensi spesifik, dimana mekanisme selektif Mamba
...
Pada XJTU-SY, terjadi pola yang berbeda {\color{blue}dan bergantung
pada arsitektur}. {\color{blue}Pada protokol lintas-arsitektur (SAE
...
{\color{blue}Pada SAE \emph{post-hoc} utama dengan anggaran pelatihan lebih
panjang (Tabel~\ref{tab:bab5_hitrate_utama}), \emph{hit-rate} XJTU-SY tersebar
pada BSF~(1{,}6\%), BPFI~(1{,}5\%), dan BPFO~(0{,}7\%), mencerminkan keragaman
mode kegagalan lintas tiga kondisi beban alih-alih dominasi satu frekuensi
tunggal.}
Hal ini mengisyaratkan bahwa pada XJTU-SY, di mana sinyal dataset tidak
membatasi urutan BPFx secara kuat, {\color{blue}bias induktif} encoder ikut
memodulasi BPFx mana yang menonjol. \emph{Gating} ruang-keadaan selektif
Mamba maupun \emph{backbone} basis-blok dan konvolusi bergerbang
menyebarkan respons ke beberapa frekuensi karakteristik sekaligus, sehingga
tidak ada satu BPFx yang mendominasi secara mutlak.

Pada validasi lintas-dataset dengan CWRU (data dengan $n = 10$ rekaman
{\color{blue}tingkat berkas}, menjadikannya \emph{underpowered}),
ketiga arsitektur secara serentak
konvergen pada pola \emph{hit-rate} BPFI-dominan dengan rentang 4{,}69--5{,}08\%.
Konvergensi ini mengisyaratkan bahwa distribusi data, bukan {\color{blue}bias induktif}
arsitektur, yang menentukan BPFx mana yang muncul dominan {\color{blue}ketika
...
{\color{blue}Temuan lintas-arsitektur ini mendukung Novelti~N4
penelitian ini dengan kualifikasi yang jujur:}
\textbf{{\color{blue}urutan dominansi BPFx terjaga secara bersih
...
ambigu}}{\color{blue}; pada XJTU-SY dan IMS, BPFx dominan berbeda
...
{\color{blue}Pelaporan konsistensi parsial ini secara terbuka justru memperkuat kredibilitas ilmiah
...
{\color{blue}Pilihan $k = 51$ (5\% sparsitas) pada SAE diuji
konsistensinya melalui \emph{sparsity sweep} atas empat nilai $k$, bukan
ditetapkan sebagai nilai optimal tunggal.}
Gambar~\ref{fig:bab5_sparsity_sweep} menampilkan perubahan \emph{hit-rate}
BPFx pada {\color{blue}keempat dataset} ketika $k$ divariasikan pada
$k \in \{10, 51, 102, 205\}$.
...
\caption{\emph{Hit-rate} BPFx sebagai fungsi dari anggaran fitur aktif $k$
         {\color{blue}untuk \emph{backbone} Mamba-xLSTM-Net. Panel kiri:
...
{\color{blue}Berbeda dengan dugaan awal yang memperkirakan adanya
...
{\color{blue}Pada PHM2012, \emph{hit-rate} naik dari sekitar
(BPFO~0{,}29\%, BPFI~0{,}68\%) pada $k = 10$ menjadi (BPFO~4{,}49\%,
BPFI~7{,}13\%) pada $k = 205$, sementara BSF dan FTF tetap nol di seluruh
rentang $k$.}
{\color{blue}Pada XJTU-SY, rentang yang setara adalah (BPFO~0{,}59\%,
BPFI~0{,}20\%) hingga (BPFO~8{,}30\%, BPFI~2{,}44\%) pada $k = 205$.}
{\color{blue}Kenaikan monoton ini mengindikasikan bahwa kamus SAE
...
{\color{blue}Pada IMS, hanya FTF yang menghasilkan \emph{hit-rate}
...
sangkar (\emph{cage}).}
{\color{blue}Pada CWRU, BPFI adalah frekuensi dominan di semua tingkat
...
{\color{blue}Berdasarkan analisis ini, $k = 51$ tidak dipilih karena
...
{\color{blue}Konsekuensinya, magnituda \emph{hit-rate} yang dilaporkan
...
Verifikasi lebih lanjut dengan $k > 205$ atau dimensi yang lebih besar
($d_{\rm lat} = 4.096$) diperlukan untuk memastikan ada-tidaknya puncak
\emph{hit-rate}.}

Selanjutnya, Bab~VI akan mensintesiskan hasil {\color{blue}komponen diagnostik} (Bab~IV) dan {\color{blue}komponen prognostik} (Bab ini)

...
%% Bab IV — Deteksi Kerusakan Bantalan: Metodologi dan Hasil
%% Sumber utama:
...
\chapter{Deteksi Kerusakan Bantalan: Metodologi dan Hasil}
\label{bab:diagnostik}

Bab ini menyajikan Jalur~A secara \emph{end-to-end}: dari perancangan
metodologi \emph{benchmark} lintas tiga keluarga algoritma hingga analisis
hasil, validasi interpretabilitas, dan sintesis lintas keluarga.
Sumber empiris utama terdiri atas tiga karya yang masing-masing merepresentasikan
satu keluarga algoritma: \citetitb{TotoSuharto2024Conf1SVM} untuk keluarga
kernel (SVM dan Regresi Logistik), \citetitb{TotoSuharto2024Conf2Tree} untuk
keluarga \emph{tree} (DT, RF, XGBoost), dan
\citetitb{TotoSuharto2025Journal1FSM} untuk keluarga jaringan dalam (WDCNN)
beserta formalisasi \emph{Fault Signature Maps} (FSM).

Fondasi bersama bab ini — dataset CWRU, prosedur praproses sinyal, vektor
...
spesifik tiap keluarga dan menyajikan hasil eksperimen beserta interpretasinya.
...
sumber data tunggal dan merancang dua alur paralel sesuai dengan representasi
input yang sesuai per keluarga model.
Alur pertama mengekstraksi vektor fitur HI~36-D dari setiap segmen sinyal
sebelum memasukkannya ke keluarga kernel dan \emph{tree}, karena kedua
keluarga ini beroperasi pada ruang fitur berdimensi tetap.
...
adaptif otomatis tanpa memerlukan rekayasa fitur manual.
...
\begin{tikzpicture}[
    ...
    xai/.style  = {rectangle, rounded corners=3pt, draw, thick, fill=yellow!15,
                   ...
                   font=\small},
    out/.style  = {rectangle, rounded corners=3pt, draw, thick, fill=orange!15,
                   align=center, minimum height=0.80cm, minimum width=2.4cm,
                   font=\small},
  ...
  \node[out, right=0.9cm of shapft] (rankf)
  ...
  \node[out, right=0.9cm of shapd] (fsm)
...
Pemilihan dua alur paralel — bukan satu alur tunggal — mencerminkan
perbedaan mendasar dalam desain model.
Keluarga kernel dan \emph{tree} dirancang untuk beroperasi pada perangkat
dengan sumber daya terbatas (\emph{edge} IoT) karena membutuhkan lebih
sedikit memori dan komputasi inferensi dibandingkan jaringan dalam.
Sebaliknya, WDCNN dioptimalkan untuk akurasi maksimum dengan interpretabilitas
level sinyal yang lebih kaya — kemampuan yang tidak dimiliki keluarga kernel
dan \emph{tree} pada representasi sinyal mentah.
Perbandingan lintas kedua alur pada §IV.14 mengungkap implikasi desain
sistem PdM multi-tier yang lebih luas.

% =================================================================
\section{Benchmark Keluarga Kernel: SVM dan Regresi Logistik}
\label{sec:bab4_kernel}
% =================================================================

Keluarga kernel terdiri atas dua model: \emph{Support Vector Machine} (SVM)
...
\subsection{Ekstraksi Fitur untuk Keluarga Kernel}
...
% =================================================================
\section{Benchmark Keluarga Tree: DT, RF, dan XGBoost}
\label{sec:bab4_tree}
% =================================================================

Keluarga \emph{tree} mencakup tiga algoritma yang merepresentasikan tiga
...
Ketiga model beroperasi pada vektor fitur HI~36-D yang sama dengan keluarga
kernel, mengikuti protokol \emph{split} dan normalisasi identik.
...
bantalan dari sinyal mentah \citetitb{Zhang2017WDCNN}.
...
\begin{enumerate}
  ...
  \item \textbf{Varian B — Kernel sempit:} Ukuran kernel pertama diperkecil
        dari 64 ke 3, menyamakan dengan kernel blok lainnya. Tujuan ablasi
        ini adalah menguji peran kernel lebar dalam pemrosesan frekuensi
        dan mengestimasi \emph{footprint} parameter setelah pengurangan
        sebesar faktor $7{,}3\times$.
...
% =================================================================
\section{Analisis SHAP per Keluarga Algoritma}
\label{sec:bab4_shap}
% =================================================================

Setiap keluarga algoritma menggunakan varian SHAP yang berbeda sesuai dengan
struktur modelnya.
...
\subsection{SHAP KernelExplainer untuk Keluarga Kernel}
...
\subsection{SHAP TreeExplainer untuk Keluarga Tree}
...
Perbedaan ini membuat nilai SHAP antara keluarga kernel dan \emph{tree}
tidak secara langsung dikomparasi secara kuantitatif — hanya secara kualitatif
pada tingkat ranking fitur.
...
ruang sinyal mentah berdimensi 2.048 — dimensi yang dua orde lebih besar.
...
Subbab~\ref{subsec:bab3_metrik_fsm}: diskriminabilitas
...
Monotonik mengukur apakah puncak FSM secara positional konsisten dengan
frekuensi karakteristik cacat yang diketahui dari fisika bantalan (BPFO,
BPFI, BSF) — metrik ini menghubungkan output SHAP dengan domain pengetahuan
fisika.
Stabilitas mengukur konsistensi FSM antar-\emph{fold} validasi silang —
FSM yang stabil berarti pola interpretasi tidak bergantung pada variasi
partisi data tertentu.
...
% =================================================================
\section{Hasil Keluarga Kernel: SVM dan Regresi Logistik}
\label{sec:bab4_hasil_kernel}
% =================================================================

Tabel~\ref{tab:bab4_hasil_kernel} merangkum akurasi uji keluarga kernel pada
dataset CWRU sepuluh kelas.

\begin{table}[ht]
\caption{Akurasi uji keluarga kernel pada dataset CWRU sepuluh kelas.}
...
\midrule
SVM-RBF      & 97,60 & 0,978 & 0,977\\
LR (OvR)     & 95,33 & 0,955 & 0,953\\
\bottomrule
\end{tabular}
\end{table}

SVM-RBF mencapai akurasi 97,60\% pada \emph{test set}, melampaui LR-OvR
sebesar 2,27 poin persentase.
...
\subsection{Interpretasi SHAP Keluarga Kernel}
...
Kurtosis mendominasi karena cacat bantalan menghasilkan impuls transien yang
...
% =================================================================
\section{Hasil Keluarga Tree: DT, RF, dan XGBoost}
\label{sec:bab4_hasil_tree}
% =================================================================

Tabel~\ref{tab:bab4_hasil_tree} menyajikan akurasi uji ketiga model keluarga
\emph{tree}.

\begin{table}[ht]
\caption{Akurasi uji keluarga \emph{tree} pada dataset CWRU sepuluh kelas.}
...
Random Forest & 98,27 & 0,983 & 0,983\\
XGBoost       & 98,80 & 0,988 & 0,988\\
\bottomrule
\end{tabular}
\end{table}

XGBoost mencapai akurasi tertinggi di antara keluarga \emph{tree} dengan
98,80\%, disusul RF pada 98,27\% dan DT pada 92,40\%.
Kesenjangan antara DT (92,40\%) dan RF (98,27\%) — selisih 5,87 poin
...
XGBoost melampaui RF sebesar 0,53 poin persentase, keunggulan yang berasal
...
\subsection{Interpretasi SHAP Keluarga Tree}
\label{subsec:bab4_shap_hasil_tree}

Gambar~\ref{fig:bab4_shap_tree} menampilkan \emph{summary plot} SHAP
\emph{TreeExplainer} untuk XGBoost — model dengan akurasi tertinggi dalam
keluarga \emph{tree}.
...
\emph{shap interaction values} yang merekam kontribusi sendi dua fitur
secara eksplisit.
...
indikasi bahwa aturan \emph{if-then} yang dihasilkan DT lebih sensitif
terhadap nilai fitur tertentu dan kurang stabil dibandingkan RF atau XGBoost.
Temuan ini konsisten dengan akurasi DT yang lebih rendah: DT menggunakan
batas keputusan yang lebih tajam dan rigid, sehingga kelas yang borderline
— seperti kelas IR-14 dan OR-14 yang sering salah diklasifikasikan —
mendapat atribusi SHAP yang lebih tidak konsisten.

Perbandingan antara keluarga kernel dan \emph{tree} pada tingkat interpretasi
SHAP menunjukkan bahwa meski fitur-fitur teratas serupa (kurtosis, RMS,
energi pita), urutan dan besaran nilai SHAP berbeda karena perbedaan
\emph{baseline} distribusi yang digunakan oleh \emph{KernelExplainer} dan
\emph{TreeExplainer}.
Oleh karena itu, perbandingan kuantitatif nilai SHAP lintas keluarga dibatasi
pada tingkat kualitatif (apakah fitur~$j$ masuk dalam sepuluh besar untuk
kedua keluarga), bukan perbandingan besaran angka secara langsung.
...
Tabel~\ref{tab:bab4_hasil_wdcnn} merangkum akurasi uji ketiga varian WDCNN
pada dataset CWRU sepuluh kelas.

\begin{table}[ht]
\caption{Akurasi uji tiga varian WDCNN pada dataset CWRU sepuluh kelas.
         Selang kepercayaan 95\% dihitung dari 5-\emph{fold} validasi silang
         berlapis.}
...
\midrule
A — Baseline (BN aktif) & 99,87 & 0,999 & 0,999 & 0,999\\
B — Kernel sempit (k=3) & 97,20 & 0,972 & 0,972 & 0,972\\
C — Tanpa BatchNorm     & 98,40 & 0,984 & 0,984 & 0,984\\
\bottomrule
\end{tabular}
\end{table}

Varian~A (WDCNN lengkap dengan BN aktif) mencapai akurasi 99,87\% —
hanya satu dari 750 sampel uji yang salah diklasifikasikan.
Akurasi ini melampaui seluruh model keluarga kernel dan \emph{tree} sebesar
minimal 1,07 poin persentase (dibandingkan XGBoost 98,80\%), sekaligus
mengkonfirmasi keunggulan pemrosesan sinyal mentah secara \emph{end-to-end}
dibandingkan rekayasa fitur manual untuk kondisi beban yang beragam.

Varian~B (kernel pertama diperkecil ke $k=3$) mengalami penurunan akurasi
menjadi 97,20\% — penurunan 2,67 poin persentase dari Varian~A.
Penurunan ini terjadi karena kernel berukuran 3 pada lapisan pertama tidak
mampu menangkap perioda impuls cacat bantalan yang lazimnya mencakup
50–100 titik sampel pada frekuensi pengambilan sampel 48~kHz.
Kernel lebar berukuran 64 dengan \emph{stride} 16 memungkinkan model
mempelajari filter frekuensi rendah secara adaptif dari data — kemampuan
yang hilang ketika digantikan kernel sempit.

Varian~C (BN dihapus seluruhnya) mencapai akurasi 98,40\% — berada di
antara Varian~A dan Varian~B.
Fakta bahwa menghapus BN menghasilkan akurasi lebih rendah daripada Varian~A
tetapi lebih tinggi daripada Varian~B menunjukkan bahwa BN tidak berfungsi
semata-mata sebagai stabilisator pelatihan, melainkan juga sebagai komponen
yang memengaruhi representasi fitur internal.
Implikasi BN terhadap interpretabilitas FSM dianalisis lebih jauh pada §IV.13.
...
Varian~B tidak disertakan dalam analisis SHAP karena penurunan akurasiny
yang substansial menjadikan nilai atribusinya sulit diinterpretasi — atribusi
dari model yang salah bukan mencerminkan pengetahuan domain yang bermakna.
...
\caption{Nilai atribusi SHAP \emph{DeepExplainer} kelas IR-21 pada 2.048
         ...
         dari fisika bantalan; lebar sumbu-$y$ mencerminkan nilai maksimum
         atribusi.
         \citetitb{TotoSuharto2025Journal1FSM}}
...
tajam dan terlokalisasi pada posisi-posisi yang bersesuaian dengan perioda
impuls BPFI — model tanpa BN mengembangkan detektor impuls yang lebih
spesifik.
...
lebih tajam dan lebih selaras dengan frekuensi BPFI dan BPFO dibandingkan
Varian~A — indikasi bahwa WDCNN tanpa BN mengembangkan representasi yang
lebih spesifik terhadap periodisitas cacat.
...
(diskriminabilitas, monotonik, stabilitas) untuk $\text{FSM}^{\text{abs}}$
dari Varian~A dan Varian~C, dirata-rata atas sepuluh kelas CWRU.

\begin{table}[ht]
\caption{Metrik validasi FSM$^{\text{abs}}$ rata-rata sepuluh kelas CWRU
         untuk Varian~A (BN aktif) dan Varian~C (tanpa BN).
         Nilai lebih tinggi lebih baik untuk diskriminabilitas dan monotonik;
         nilai lebih tinggi lebih baik untuk stabilitas (konsistensi).}
...
\midrule
A — BN aktif   & 0,216 & 0,940 & 0,873\\
C — Tanpa BN   & 0,735 & 0,962 & 0,891\\
\bottomrule
\end{tabular}
\end{table}

Hasil kuantitatif menunjukkan perbedaan yang signifikan pada metrik
diskriminabilitas: Varian~C menghasilkan diskriminabilitas 0,735, dibandingkan
Varian~A yang hanya 0,216 — peningkatan 0,519 poin atau sekitar +133\%
\citetitb{TotoSuharto2025Journal1FSM}.
Artinya, FSM yang dihasilkan WDCNN tanpa BN jauh lebih mudah dibedakan
antarkelas dibandingkan FSM dari WDCNN dengan BN, meskipun akurasi klasifikasi
Varian~A lebih tinggi (99,87\% vs 98,40\%).

Dua temuan ini menciptakan apa yang disebut \emph{interpretability–accuracy
trade-off}: BN meningkatkan akurasi klasifikasi tetapi menurunkan
diskriminabilitas FSM.
Temuan ini merupakan inti Novelti~N2 penelitian ini — suatu hubungan
\emph{trade-off} yang sebelumnya tidak terdokumentasi antara \emph{Batch
Normalization} dan kualitas interpretabilitas pada level sinyal.

Metrik monotonik Varian~C (0,962) sedikit lebih tinggi daripada Varian~A
(0,940), mengindikasikan bahwa WDCNN tanpa BN menghasilkan puncak FSM yang
sedikit lebih selaras dengan frekuensi karakteristik fisika bantalan.
Metrik stabilitas juga sedikit lebih baik pada Varian~C (0,891 vs 0,873),
bertentangan dengan intuisi bahwa menghapus BN akan membuat model kurang
stabil.
Penjelasan parsial untuk fenomena ini adalah bahwa BN, dengan meredistribusi
aktivasi internal, menciptakan kergantungan pada statistik \emph{batch}
yang dapat berfluktuasi lintas \emph{fold}, sehingga justru memperkenalkan
variabilitas yang tidak ada pada Varian~C.
...
% =================================================================
\section{Interpretasi Fisika: Korespondensi FSM dengan Frekuensi Cacat}
\label{sec:bab4_fisika}
% =================================================================

Validasi fisika FSM bertujuan memverifikasi bahwa puncak-puncak FSM yang
diidentifikasi secara statistik benar-benar bersesuaian dengan frekuensi
karakteristik cacat bantalan yang dapat diturunkan dari geometri bantalan —
yaitu frekuensi BPFO, BPFI, BSF, dan FTF sebagaimana didefinisikan pada
Subbab~\ref{subsec:bab3_frekuensi_cacat}.

Dari geometri bantalan CWRU (8 bola, sudut kontak 15,17°, diameter bola
0,3126 inci, diameter \emph{pitch} 1,537 inci), frekuensi karakteristik pada
kecepatan putar 1.750 rpm adalah:
BPFO $\approx$ 104,6~Hz, BPFI $\approx$ 157,9~Hz, BSF $\approx$ 68,9~Hz,
dan FTF $\approx$ 13,1~Hz.
Pada frekuensi pengambilan sampel 48~kHz dan panjang segmen 2.048 titik,
perioda impuls BPFI setara dengan 304 titik sampel — posisi yang dapat
diverifikasi secara eksak pada FSM.

\subsection{Korespondensi Puncak FSM dengan BPFO dan BPFI}
...
posisi BPFO dan BPFI yang diharapkan secara teoritis.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{figures/bab4/fsm_physics.pdf}
\caption{$\text{FSM}^{\text{abs}}$ Varian~C (tanpa BN) untuk kelas IR-7,
         IR-14, IR-21, dan OR-21. Garis vertikal biru menandai posisi BPFI
         yang diharapkan secara teoritis; garis merah menandai BPFO.
         Puncak FSM yang bersesuaian dengan garis teoritis mengkonfirmasi
         korespondensi fisika.
         \citetitb{TotoSuharto2025Journal1FSM}}
\label{fig:bab4_fsm_fisika}
\end{figure}

Tiga pengamatan utama dari analisis korespondensi fisika tersebut adalah
sebagai berikut.
Pertama, FSM kelas IR-7, IR-14, dan IR-21 memperlihatkan puncak utama yang
konsisten bertepatan dengan posisi BPFI teoritis, dengan kesalahan posisi
rata-rata kurang dari 5 titik sampel ($< 0{,}1$~ms) — akurasi yang
menunjukkan bahwa WDCNN secara efektif belajar mengenali periodisitas BPFI
sebagai ciri pembeda kelas IR.
Kedua, FSM kelas OR-21 memperlihatkan puncak yang bertepatan dengan BPFO,
sebagaimana diharapkan dari fisika cacat \emph{outer race} yang terakselerasi
pada frekuensi lebih rendah daripada cacat \emph{inner race}.
Ketiga, amplitudo puncak FSM meningkat seiring tingkat keparahan cacat:
FSM-IR-21 memiliki puncak BPFI yang lebih tajam daripada FSM-IR-7,
mencerminkan fakta bahwa cacat yang lebih parah menghasilkan impuls yang
lebih kuat dan lebih mudah dikenali oleh model.

Korespondensi FSM dengan frekuensi karakteristik fisika ini memberikan
validasi eksternal yang kuat bahwa model WDCNN tidak sekadar menghafal
statistik permukaan dataset CWRU, melainkan benar-benar mengekstraksi
informasi yang relevan secara fisika dari sinyal getaran.
Temuan ini menjawab RQ1 penelitian ini sebagaimana dirumuskan pada
§I.3: interpretabilitas berbasis SHAP yang diformalisasi sebagai FSM
dapat dikaitkan dengan pengetahuan domain fisika bantalan secara kuantitatif.
...
\caption{Ringkasan metrik kinerja dan interpretabilitas FSM$^{\text{abs}}$
         tiga varian WDCNN. Kolom \textbf{F1} adalah F1 makro; kolom
         \textbf{Disk.}, \textbf{Mon.}, dan \textbf{Stab.} adalah
         diskriminabilitas, monotonik, dan stabilitas FSM$^{\text{abs}}$.
...
\textbf{Varian} & \textbf{Akurasi (\%)\,$\uparrow$} & \textbf{F1\,$\uparrow$}
                & \textbf{Disk.\,$\uparrow$} & \textbf{Mon.\,$\uparrow$}
                & \textbf{Stab.\,$\uparrow$}\\
\midrule
A — BN aktif    & \textbf{99,87} & \textbf{0,999} & 0,216 & 0,940 & 0,873\\
B — Kernel $k=3$& 97,20 & 0,972 & — & — & —\\
C — Tanpa BN    & 98,40 & 0,984 & \textbf{0,735} & \textbf{0,962} & \textbf{0,891}\\
\bottomrule
\end{tabular}
\end{table}
...
(akurasi dan F1), sementara Varian~C unggul pada ketiga metrik interpretabilitas
FSM.
Varian~B tidak disertakan dalam evaluasi FSM karena penurunan akurasinya
yang terlalu substansial menjadikan analisis interpretabilitas tidak bermakna.
...
% =================================================================
\section{Sintesis Lintas Keluarga dan Implikasi Desain PdM}
...
\begin{table}[ht]
\caption{Perbandingan lintas keluarga algoritma diagnostik dari perspektif
...
\midrule
SVM-RBF         & 97,60 & — (fitur 36-D) & SHAP Kernel & $O(n_\text{sv})$    & Edge IoT $\star$\\
LR-OvR          & 95,33 & — (fitur 36-D) & SHAP Kernel & $O(d)$              & Edge IoT\\
Decision Tree   & 92,40 & — (fitur 36-D) & \emph{Rules} & $O(\text{depth})$  & Edge IoT\\
Random Forest   & 98,27 & — (fitur 36-D) & SHAP Tree   & $O(T \cdot \text{depth})$ & Edge Server\\
XGBoost         & 98,80 & — (fitur 36-D) & SHAP Tree   & $O(T \cdot \text{depth})$ & Edge Server $\star$\\
WDCNN Var-A     & 99,87 & $\sim$60.710   & FSM (rendah) & $O(L)$ GPU          & Cloud/GPU $\star$\\
WDCNN Var-C     & 98,40 & $\sim$60.710   & FSM (tinggi) & $O(L)$ GPU          & Cloud/GPU\\
\bottomrule
\end{tabular}
\end{table}

Tiga kesimpulan utama dari sintesis lintas keluarga ini adalah sebagai
berikut.
Pertama, terdapat gradien akurasi yang jelas antara keluarga: WDCNN~Var-A
(99,87\%) $>$ XGBoost (98,80\%) $>$ RF (98,27\%) $>$ SVM-RBF (97,60\%) $>$
LR-OvR (95,33\%) $>$ DT (92,40\%).
Gradien ini mencerminkan peningkatan kapasitas representasi dari model
linear ke model ensemble ke model jaringan dalam.

Kedua, biaya komputasi dan memori juga meningkat seiring kapasitas:
DT/SVM/LR beroperasi pada mikrokontroler dengan RAM di bawah 1~MB, sedangkan
WDCNN membutuhkan GPU untuk inferensi yang dapat memenuhi persyaratan
\emph{real-time} perawatan kondisi di lini produksi.
...
Keluarga kernel dan \emph{tree} lainnya memberikan interpretabilitas fitur
melalui SHAP pada ruang HI~36-D.
WDCNN memberikan interpretabilitas level sinyal melalui FSM — representasi
yang lebih kaya tetapi dengan kompleksitas analisis yang lebih tinggi.
...
mesin vs insinyur pemeliharaan vs peneliti).

Bab~V akan mengembangkan Jalur~B (prognostik) dengan memperkenalkan tiga

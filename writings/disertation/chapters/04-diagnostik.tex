...
menghubungkan output SHAP dengan sifat fisika degradasi bantalan.
...
lebih spesifik secara positional.
...
rata-rata CV Varian~A lebih tinggi (99,73\% vs 96,13\%).
...
aktivasi internal, menciptakan ketergantungan pada statistik \emph{batch}
yang dapat berfluktuasi lintas \emph{fold}, sehingga justru memperkenalkan
variabilitas yang tidak ada pada Varian~C.

Analisis tambahan terhadap metrik monotonik per kelompok kelas mengungkap
pola yang konsisten dengan fisika keparahan cacat.
...
teoritis frekuensi karakteristik cacat bantalan (BPFO, BPFI, BSF, FTF) yang
diturunkan dari geometri bantalan.
...
pada kecepatan putar 1.750 rpm adalah BPFO $\approx$ 104,6~Hz,
BPFI $\approx$ 157,9~Hz, BSF $\approx$ 68,9~Hz, dan FTF $\approx$ 13,1~Hz.
Pada frekuensi pengambilan sampel 48~kHz dan panjang segmen 2.048 titik,
perioda impuls BPFI setara dengan 304 titik sampel.
...
tidak berulang secara periodik pada kelipatan BPFI (304 titik sampel) maupun
BPFO.
...
Dengan kata lain, WDCNN mengklasifikasikan kerusakan bantalan berdasarkan
...
Karena WDCNN tidak menggunakan geometri bantalan secara implisit dalam
mekanisme klasifikasinya, model yang dilatih pada CWRU berpotensi ditransfer
ke jenis bantalan lain dengan parameter geometri berbeda tanpa perlu
...
\caption{Ringkasan metrik kinerja dan interpretabilitas FSM$^{\text{abs}}$
         tiga varian WDCNN. Akurasi adalah rata-rata 5-\emph{fold} validasi
         silang (berbeda dari akurasi uji tunggal Varian~A = 99,87\% pada
         Tabel~\ref{tab:bab4_hasil_wdcnn}). Kolom \textbf{F1} adalah F1
         makro; kolom \textbf{Disk.}, \textbf{Mon.}, dan \textbf{Stab.}
         adalah diskriminabilitas, monotonik, dan stabilitas FSM$^{\text{abs}}$.
...
\textbf{Varian} & \textbf{Akurasi (\%)\,$\uparrow$} & \textbf{F1\,$\uparrow$}
                ...
                & \textbf{Stab.\,$\uparrow$}\\
\midrule
A (Baseline, BN aktif) & \textbf{99,73} & \textbf{0,997} & 0,216 & 0,940 & 0,873\\
B (Kernel $k=3$)       & 98,93 ($\downarrow$0,80) & 0,989 & \multicolumn{3}{c}{tidak dievaluasi}\\
C (Tanpa BN)           & 96,13 ($\downarrow$3,60) & 0,961 & \textbf{0,735} & \textbf{0,962} & \textbf{0,891}\\
\bottomrule
\end{tabular}
\end{table}
...
(akurasi dan F1 rata-rata 5-\emph{fold}), sementara Varian~C unggul pada
ketiga metrik interpretabilitas FSM.
Perbedaan antara akurasi uji tunggal Varian~A (99,87\% pada
Tabel~\ref{tab:bab4_hasil_wdcnn}) dan akurasi rata-rata CV (99,73\% pada
tabel ini) mencerminkan variansi alami antar partisi data.
Varian~B tidak disertakan dalam evaluasi FSM karena penurunan akurasinya
yang substansial menjadikan analisis interpretabilitas tidak bermakna.

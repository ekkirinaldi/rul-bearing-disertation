...
%% Sumber utama:
%%   Conference 1 — SVM/LR + SHAP KernelExplainer (Subbab~IV.2, Subbab~IV.7)
%%   Conference 2 — DT/RF/XGBoost + SHAP TreeExplainer (Subbab~IV.3, Subbab~IV.8)
%%   Journal 1    — WDCNN + SHAP DeepExplainer + FSM (Subbab~IV.4–IV.6, Subbab~IV.9–IV.14)
%% Fase D — dibuat sesuai writings/dissertation-outline.md Bab IV

\chapter{Deteksi Kerusakan \emph{Bearing}}
\label{bab:diagnostik}

Bab ini menyajikan {\color{blue}komponen diagnostik} dari penelitian ini secara \emph{end-to-end}: mulai dari perancangan
metodologi, perbandingan antara tiga kelompok algoritma hingga analisis
hasil, validasi interpretabilitas, dan sintesis antar kelompok.
Sumber empiris utama pembahasan bab ini terdiri atas tiga publikasi yang masing-masing merepresentasikan
...
\citetitb{TotoSuharto2025Journal1FSM} untuk arsitektur \emph{deep network} WDCNN
beserta formalisasi \emph{Fault Signature Maps} (FSM).

Fondasi konsep pada bab ini, yaitu dataset CWRU, prosedur praproses sinyal, vektor
...
Subbab~IV.1 s.d.\ Subbab~IV.6 membahas metodologi; Subbab~IV.7 s.d.\ Subbab~IV.14
membahas hasil dan analisis.

% =================================================================
\section{Pipeline Diagnostik CWRU}
\label{sec:bab4_pipeline}
% =================================================================

Komponen diagnostik pada penelitian ini menggunakan dataset CWRU sebagai
...
\caption{Pipeline diagnostik CWRU dua alur. Alur~1 (biru) mengekstraksi vektor
         fitur HI~36-D sebelum melatih model kernel
         SHAP \emph{KernelExplainer} dan \emph{TreeExplainer} untuk menghasilkan
         atribusi di ruang fitur. Alur~2 (hijau) memasukkan sinyal dasar
...
lebih sedikit memori dan komputasi inferensi dibandingkan \emph{deep network}.
...
Perbandingan lintas kedua alur pada Subbab~IV.14 mengungkap implikasi desain
sistem PdM multi-tier yang lebih luas.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.78\textwidth]{figures/bab4/j1_overall_methodology.png}
  \caption{Alur metodologi diagnostik keseluruhan dari Suharto dkk. (2025):
           empat tahap sekuensial---persiapan data CWRU, pelatihan WDCNN,
           analisis SHAP \emph{DeepExplainer}, dan konstruksi serta validasi
           \emph{Fault Signature Maps}---yang menjadi referensi implementasi
           eksperimen diagnostik pada bab ini.
           \citetitb{TotoSuharto2025Journal1FSM}}
  \label{fig:bab4_j1_methodology}
\end{figure}
...
Pada subbab ini, dua model berbasis kernel dievaluasi: \emph{Support Vector Machine} (SVM)
dengan kernel RBF dan Regresi Logistik (\emph{Logistic Regression}, LR) untuk klasifikasi sepuluh kelas.
...
Dari setiap segmen sinyal dataset CWRU sepanjang 2.048 titik, diekstraksi
...
Selain itu, diekstraksi juga \textbf{sembilan fitur domain frekuensi} per kanal:
...
adalah $9 \times 2 + 9 \times 2 = 36$, sesuai dengan definisi lengkap
pada Lampiran~\ref{lamp:fitur}.

Setelah diekstraksi, vektor HI~36-D dinormalisasi ulang menggunakan Z-score
...
distribusi \emph{zero-mean unit-variance} pada ruang input model.
Normalisasi ini bersifat wajib untuk model SVM-RBF agar jarak antar sampel dalam
...
ditetapkan untuk memastikan kelas-kelas yang seimbang diakomodasi secara merata.
...
\emph{cross-validation} pada \emph{training set} sebelum dievaluasi pada
\emph{test set}.
...
berbasis kernel, mengikuti protokol \emph{split} dan normalisasi yang identik.

\subsection{Konfigurasi Decision Tree}
\label{subsec:bab4_dt_config}

Model DT yang dipilih dilatih dengan kriteria pemisahan dan dibatasi kedalaman maksimum
...
strukturalnya yang paling tinggi di antara ketiga model, dimana setiap keputusan
...
Model RF yang dipilih menggunakan 100 pohon (\texttt{n\_estimators=100}) dengan kedalaman
...
Metode \emph{bagging} terhadap 100 pohon mereduksi variansi prediksi secara signifikan
dibandingkan DT tunggal, sehingga RF umumnya menghasilkan akurasi lebih
tinggi namun dengan biaya komputasi yang jauh lebih besar.
...
kelas. Detail lengkap implementasi model ini tersedia di Lampiran~F.
...
Input model WDCNN merupakan sinyal mentah ternormalisasi berdimensi $1 \times 2048$
...
Gambar~\ref{fig:bab4_wdcnn_arch} memvisualisasikan seluruh blok arsitektur
WDCNN dari masukan hingga keluaran.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.94\textwidth]{figures/bab4/wdcnn_architecture.png}
  \caption{Diagram arsitektur WDCNN \citetitb{Zhang2017WDCNN} yang digunakan
           dalam penelitian ini: input $1 \times 2.048$ diproses melalui
           lima blok konvolusi (Conv1d, BatchNorm, ReLU, MaxPool) yang
           secara progresif mengurangi dimensi temporal, diikuti dua lapisan
           \emph{fully connected} (FC) dan \emph{dropout} sebelum keluaran
           10-kelas. Sumber: \citetitb{TotoSuharto2025Journal1FSM}.}
  \label{fig:bab4_wdcnn_arch}
\end{figure}

\subsection{Tiga Varian Studi Ablasi}
...
dan penjadwal \emph{Step Learning Rate} (StepLR, $\gamma = 0{,}5$, berubah
setiap 20 \emph{epoch}).
Fungsi \emph{loss} yang dipilih adalah \emph{CrossEntropyLoss} standar tanpa bobot kelas
(kelas CWRU seimbang dengan jumlah 75 sampel per kelas pada \emph{test set}).
Metode \emph{Early stopping} dengan waktu tunggu 15 \emph{epoch} diaktifkan berdasarkan
akurasi validasi, dan \emph{checkpoint} terbaik disimpan untuk evaluasi
akhir pada \emph{test set}.
Detail hiperparameter lengkap WDCNN tersedia di Lampiran~G.
...
struktur modelnya.
Pemilihan varian ini bukan sekadar pilihan implementasi, melainkan
...
Karena SVM-RBF tidak memiliki struktur \emph{tree} maupun \emph{gradient flow}, 
maka \emph{KernelExplainer} adalah satu-satunya pilihan yang dapat
memberikan jaminan properti Shapley (efisiensi, simetri, \emph{null player},
dan aditif) tanpa asumsi arsitektur model tertentu \citetitb{TotoSuharto2024Conf1SVM}.

Sebagai \emph{background dataset}, digunakan sampel acak stratifikasi 
kecil dari \emph{training set}, sebuah strategi yang menyeimbangkan
akurasi estimasi dan waktu komputasi, karena kompleksitas \emph{KernelExplainer}
berskala kuadratik terhadap ukuran \emph{background dataset}.
Untuk setiap sampel uji, algoritma ini mengevaluasi $2^{36}$ subhimpunan fitur
...
{\color{blue}\citenameitb{Lundberg2017SHAP}}, dengan kompleksitas waktu $O(TLD^2)$ di mana $T$
...
(bukan aproksimasi), sehingga memungkinkan komputasi atribusi atas seluruh \emph{test
set} terjadi dalam hitungan detik bahkan untuk RF dengan 100 pohon.
...
ruang sinyal mentah berdimensi 2.048, jumlah yang jauh lebih besar dari ruang fitur.

Sebagai \emph{background dataset}, digunakanlah 300 sampel yang dipilih secara
stratifikasi dari \emph{training set}, dengan 30 sampel per kelas dari 10 kelas
CWRU.
...
\emph{DeepLIFT}.
Evaluasi dilakukan pada 500 sampel uji yang dipilih metode stratifikasi (50 sampel
per kelas).
...
dan konsistensi antar sampel (\emph{Variance}) \citetitb{TotoSuharto2025Journal1FSM}.

$\phi^{(c)}(\mathbf{x})[p]$ dinyatakan sebagai nilai SHAP kelas~$c$ pada
posisi sinyal~$p$ untuk sampel $\mathbf{x}$, dan $\mathcal{D}_c$ dinyatakan sebagai
...
$\text{FSM}_c^{\text{signed}}$ menangkap arah dominan kontribusi, dimana nilai positif
...
$\text{FSM}_c^{\text{var}}$ mengukur variabilitas atribusi antar sampel, dimana
...
Pembagi yang digunakan adalah $|\mathcal{D}_c|$, bukan $|\mathcal{D}_c| - 1$,
karena $\text{FSM}_c^{\text{var}}$ diperlakukan sebagai deskripsi sebaran pada
himpunan sampel yang dianalisis, bukan sebagai penduga tak-bias variansi
populasi. Pilihan ini konsisten dengan implementasinya.
...
FSM kelas lain, dimana nilai tinggi menunjukkan pola spesifik kelas yang kuat dan
tidak tumpang tindih antarkelas.
Metrik monotonik mengukur apakah amplitudo puncak FSM meningkat secara konsisten
...
Stabilitas mengukur konsistensi FSM {\color{blue}antara dua paruh acak
yang saling lepas dari sampel uji tiap kelas (\emph{split-half}) melalui
kemiripan kosinus}. FSM yang stabil menunjukkan pola interpretasi yang tidak bergantung pada variasi
{\color{blue}pemilihan sampel} tertentu.
...
Hasil evaluasi disajikan pada Subbab~IV.11.
...
batas keputusan non-linear di ruang fitur HI~36-D, sedangkan model LR
terbatas pada batas keputusan linear per pasangan kelas-OvR.
...
Fitur kurtosis mendominasi karena cacat \emph{bearing} menghasilkan impuls transien yang
...
\citetitb{TotoSuharto2024Conf2Tree}.
Selisih 3,4 poin persentasre antara DT (92,40\%) dan RF (95,8\%)
 mencerminkan reduksi variansi yang diperoleh \emph{bagging}
atas 100 pohon: setiap pohon DT tunggal \emph{overfit} terhadap detail
tertentu dari \emph{training set}, sedangkan model RF merata-rata kesalahan
\emph{overfitting} tersebut.
Model XGBoost melampaui RF sebesar 1,3 poin persentase berkat mekanisme \emph{boosting},
dimana setiap pohon baru difokuskan pada sampel
...
\emph{shap interaction values} yang merekam {\color{blue}kontribusi gabungan dua fitur}
secara eksplisit.
...
Oleh karena itu, perbandingan kuantitatif nilai SHAP antar kedua kelompok
...
{\color{blue}WDCNN dilatih dan dievaluasi mengikuti pembagian temporal
...
Varian~B dan Varian~C dilaporkan secara komparatif pada studi ablasi di Subbab~IV.13.
...
WDCNN Varian~A mencapai akurasi 99,87\% pada \emph{test set} dengan hanya satu
...
adaptif yang mengoptimalkan pemisahan antar sepuluh kelas kerusakan secara
serentak.
...
WDCNN Varian~A konvergen pada \emph{epoch} ke-47 dan menunjukkan kurva pelatihan
yang halus dan monoton meningkat, sebuah karakteristik model yang stabil secara
...
Sementara, Varian~B menunjukkan plateau konvergensi tercepat (epoch ke-32) tetapi pada
level akurasi yang paling rendah, mengindikasikan bahwa model dengan kernel
sempit memiliki kapasitas representasi yang terbatas bukan karena \emph{underfitting}
...
lebih spesifik {\color{blue}secara posisional}.
...
tidak adanya komponen impuls periodik pada sinyal normal.
...
{\color{blue}(diskriminabilitas, monotonik, stabilitas) untuk
...
perbandingan terhadap Varian~C dibahas tersendiri pada studi ablasi Subbab~IV.13.}

\begin{table}[ht]
\caption{{\color{blue}Metrik validasi FSM$^{\text{abs}}$ Varian~A
...
\midrule
{\color{blue}A (BN aktif)} & {\color{blue}0,216} & {\color{blue}0,940} & {\color{blue}0,940}\\
\bottomrule
\end{tabular}
\end{table}

{\color{blue}Hasil analisis utama menunjukkan bahwa FSM$^{\text{abs}}$
...
mengindikasikan bahwa {\color{blue}FSM rata-rata antarkelas justru relatif
...
{\color{blue}Karena itu, kekhasan FSM lebih efektif dinilai secara visual
melalui \emph{heatmap} yang menyoroti konsentrasi temporal spesifik, bukan
semata melalui metrik kemiripan kosinus 2.048-D. Studi ablasi Subbab~IV.13
...
Metrik monotonik {\color{blue}Varian~A (0,940 untuk FSM$^{\text{abs}}$
agregat)} menghubungkan output SHAP dengan fisika degradasi \emph{bearing}.
{\color{blue}Analisis monotonik per kelompok kelas mengungkap pola yang
...
(variansi rendah menandakan pola yang dapat diandalkan).
...
pada kecepatan putar {\color{blue}1.772~rpm (CWRU Load~1)} adalah
{\color{blue}BPFO $\approx$ 105,9~Hz, BPFI $\approx$ 159,9~Hz, dan
BSF $\approx$ 69,6~Hz}.
Pada frekuensi pengambilan sampel 48~kHz dan panjang segmen 2.048 titik,
perioda impuls BPFI setara dengan {\color{blue}$\approx 300$ titik sampel}.
Seandainya WDCNN mengklasifikasikan berdasarkan periodisitas BPFx, maka puncak-puncak
...
({\color{blue}$\approx 300$ titik sampel}) maupun BPFO.
...
Subbab~IV.13), konsisten dengan hipotesis bahwa kernel lebar berukuran 64 diperlukan
...
yang bersifat \emph{transferable}, berbeda dari metode spektrum amplop yang secara
...
\caption{Ringkasan metrik kinerja dan interpretabilitas FSM$^{\text{abs}}$
         tiga varian WDCNN. {\color{blue}Akurasi diperoleh dari protokol
         ...
         makro; {\color{blue}kolom \textbf{Param.} adalah jumlah parameter
...
\textbf{Varian} & \textbf{Akurasi (\%)\,$\uparrow$} & \textbf{F1\,$\uparrow$}
                & {\color{blue}\textbf{Param.}}
                & \textbf{Disk.\,$\uparrow$} & \textbf{Stab.\,$\uparrow$}\\
\midrule
A (Baseline, BN aktif) & \textbf{99,73} & \textbf{0,997} & {\color{blue}60.710} & {\color{blue}0,315} & {\color{blue}0,705}\\
B (Kernel $k=3$)       & 98,93 ($\downarrow$0,80) & 0,989 & {\color{blue}443.734} & {\color{blue}0,369} & {\color{blue}0,690}\\
C (Tanpa BN)           & 96,13 ($\downarrow$3,60) & 0,961 & {\color{blue}60.230} & {\color{blue}\textbf{0,735}} & {\color{blue}\textbf{0,741}}\\
\bottomrule
\end{tabular}
\end{table}
...
(akurasi dan F1), sementara {\color{blue}Varian~C unggul pada
diskriminabilitas FSM}.
{\color{blue}Perbedaan antara akurasi uji Varian~A pada
...
{\color{blue}Penghapusan \emph{Batch Normalization} pada Varian~C
...
{\color{blue}Penggantian kernel pertama lebar (64) dengan kernel sempit
...
% =================================================================
\section{Sintesis antar Kelompok Algoritma dan Implikasi Desain PdM}
\label{sec:bab4_sintesis}
% =================================================================

Tabel~\ref{tab:bab4_sintesis} merangkum posisi komparatif seluruh model
yang dievaluasi pada Bab ini dari perspektif PdM \emph{multi-tier}, dengan mempertimbangkan
akurasi, biaya komputasi inferensi, kebutuhan memori, dan kualitas
interpretabilitas.

\begin{table}[ht]
\caption{Perbandingan antar kelompok model diagnostik dari perspektif
...
Tiga kesimpulan utama dari sintesis antar kelompok algoritma ini adalah
sebagai berikut.
Pertama, terdapat gradien akurasi yang konsisten antara WDCNN~Var-A (99,87\%)
...
Kedua, biaya komputasi dan memori juga meningkat seiring kapasitas, dimana jika
DT, SVM, dan LR dapat beroperasi pada mikrokontroler dengan RAM di bawah 1~MB, maka
...
Arsitektur PdM multi-tier pada Bab~V memanfaatkan gradien ini dengan
...
Ketiga, interpretabilitas bukan konsep yang biner.
...
tiga dimensi, yaitu akurasi yang dibutuhkan, sumber daya komputasi yang tersedia,
dan kedalaman interpretabilitas yang diperlukan oleh pengguna akhir (operator
mesin, insinyur pemeliharaan, atau peneliti).

Bab~V akan mengembangkan {\color{blue}komponen prognostik} dengan memperkenalkan tiga

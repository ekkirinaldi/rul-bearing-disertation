...
% Perubahan signifikan dan referensi baru ditandai {\color{blue}...}
% =====================================================================
\chapter{Tinjauan Pustaka}
\label{bab:tinjauan-pustaka}

Bab ini membahas tinjauan terhadap literatur yang mendasari penelitian,
{\color{blue}disusun dalam delapan sub bagian:
...
pada Subbab~\ref{sec:bab2_gap_literatur} menjadi {\color{blue}justifikasi langsung bagi
ketiga rumusan masalah} yang dirumuskan pada {\color{blue}Subbab~\ref{sec:bab1_rumusan}}.

% Catatan perubahan struktur:
% - Subbab~II.6 (fisika vibrasi) DIPINDAHKAN menjadi Subbab~II.2 agar frekuensi
%   karakteristik sudah terdefinisi sebelum digunakan di Subbab~II.3 (diagnostik).
% - Subbab~II.7 baru ditambahkan: tinjauan XAI lintas domain.
...
ini, sebagaimana diuraikan pada Subbab~VI.2.
...
ditetapkan sebagai \emph{future work} pada Subbab~VI.5.

% -----------------------------------------------------------------
% Subbab~II.2 BARU — dipindahkan dari Subbab~II.6 lama
% -----------------------------------------------------------------
{\color{blue}\section{Vibrasi \emph{Rolling Element Bearing}}
\label{sec:bab2_fisika_vibrasi}}

{\color{blue}Pemahaman terhadap fisika vibrasi \emph{rolling element bearing} memberikan \emph{ground
...
Dengan meletakkan dasar fisika sebelum bagian diagnostik (Subbab~\ref{sec:bab2_diagnostik})
dan prognostik (Subbab~\ref{sec:bab2_prognostik}), pembaca dapat mengikuti terminologi
...
{\color{blue}Untuk \emph{bearing} CWRU ($n = 8$ bola, $\alpha = 15{,}17^{\circ}$,
...
memvalidasi pemetaan SAE pada Subbab~V.3 dan untuk menginterpretasikan pola impuls
transien pada FSM di Subbab~IV.6.
{\color{blue}Nilai-nilai ini konsisten dengan geometri dan kecepatan
yang dilaporkan pada Subbab~IV.11.}
...
{\color{blue}\subsection{\emph{Transfer Learning} dan \emph{Domain Adaptation}: Tren dan Posisi Penelitian}
\label{subsec:bab2_transfer_learning}}

{\color{blue}Sejak 2020, \emph{transfer learning} dan \emph{domain adaptation} telah menjadi
...
{\color{blue}Meskipun demikian, penelitian ini tidak mengambil arah \emph{transfer learning}
...
\emph{future work} pada Subbab~VI.5.}
...
{\color{blue}Sebagai perbandingan dengan literatur sebelumnya,
...
interpretabilitas dari representasi tiap model pendekatan. {\color{blue}Angka kunci
...
Subbab~IV.7--Subbab~IV.9.

\begin{table}[ht]
  \centering
  \caption{Ringkasan kinerja dan kemampuan interpretabilitas tiga model pendekatan
           algoritma klasifikasi \emph{bearing} pada dataset CWRU{\color{blue},
  ...
  \begin{tabularx}{\textwidth}{@{}l l l c l c@{}}
    ...
    {\color{blue}SVM-RBF (Santos dkk., 2021)$^*$} & {\color{blue}Berbasis kernel} & {\color{blue}CWRU} & {\color{blue}95,1} & {\color{blue}\emph{KernelExplainer}} & {\color{blue}—} \\
    RF            & Berbasis pohon  & CWRU & 95,8  & \emph{TreeExplainer}  & — \\
    XGBoost       & Berbasis pohon  & CWRU & 97,1  & \emph{TreeExplainer}  & — \\
    {\color{blue}XGBoost (Cerrada dkk., 2022)$^*$} & {\color{blue}Berbasis pohon} & {\color{blue}CWRU} & {\color{blue}96,8} & {\color{blue}\emph{TreeExplainer}} & {\color{blue}—} \\
    WDCNN (var A) & Deep Learning & CWRU & 99,87 & \emph{DeepExplainer}  & \checkmark \\
    WDCNN (var C, tanpa BN) & Deep Learning& CWRU & {\color{blue}96,13} & \emph{DeepExplainer} & \checkmark \\
    {\color{blue}CNN-1D \citetitb{Zhang2017WDCNN}$^*$} & {\color{blue}Jaringan dalam} & {\color{blue}CWRU} & {\color{blue}99,60} & {\color{blue}—} & {\color{blue}—} \\
    \bottomrule
  \end{tabularx}
  ...
  {\footnotesize Sumber: \citenameitb{TotoSuharto2024Conf1SVM} (SVM/LR);
   ...
   di Lampiran E dan F.
   {\color{blue}($^*$) Baris perbandingan dari literatur eksternal;
...
{\color{blue}penonaktifan \emph{BatchNorm} (Varian~C) meningkatkan
...
disajikan pada Subbab~IV.6): (i)~\emph{discriminability index}: sejauh mana
...
keparahan cacat; (iii)~\emph{split-half stability coefficient}: {\color{blue}kemiripan
kosinus (\emph{cosine similarity})} antara FSM yang dihitung dari dua subset acak separuh data uji.
{\color{blue}Pada WDCNN Varian~A, koefisien stabilitas mencapai 0,940
...
{\color{blue}\subsection{Konstruksi \emph{Health Indicator} dan Normalisasi Degradasi}
\label{subsec:bab2_health_indicator}}

{\color{blue}Sebelum model \emph{deep learning} dapat dilatih untuk estimasi RUL, sinyal vibrasi
...
{\color{blue}Pendekatan berbasis statistik temporal seperti RMS (\emph{root mean square}) dan
...
{\color{blue}Pilihan metode normalisasi HI berpengaruh signifikan terhadap kemampuan model
...
RUL yang digunakan dalam penelitian ini diuraikan lebih lanjut pada Subbab~III.4.}
...
\citetitb{Tay2022LRA}. Gambar~\ref{fig:bab2_lstm_cell} mengilustrasikan
arsitektur sel LSTM klasik dengan empat mekanisme \emph{gating}.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.92\textwidth]{figures/bab2/fig_lstm_cell.png}
  \caption{Arsitektur sel LSTM klasik \citetitb{Hochreiter1997LSTM}: empat \emph{gate}
           (forget, input, candidate, output) mengatur aliran informasi melalui
           \emph{cell state} $c_t$ yang mengalir horizontal, mengatasi masalah
           \emph{vanishing gradient} pada jaringan rekuren konvensional.
           Diadaptasi dari \citetitb{Hochreiter1997LSTM}.}
  \label{fig:bab2_lstm_cell}
\end{figure}
...
untuk sekuens panjang (Gambar~\ref{fig:bab2_mamba_ssm}).
xLSTM \citetitb{Beck2024xLSTM} merevitalisasi LSTM
...
(Gambar~\ref{fig:bab2_slstm} dan \ref{fig:bab2_mlstm}).

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.92\textwidth]{figures/bab2/fig_mamba_ssm_block.png}
  \caption{Blok \emph{Selective State Space Model} (Mamba, Gu \& Dao, 2023):
           parameter $\Delta_t$, $B_t$, $C_t$ bergantung pada masukan $x_t$
           sehingga model dapat memilih informasi yang dipertahankan, ditolak,
           atau dilewatkan secara adaptif dengan kompleksitas $\mathcal{O}(L \cdot d)$.
           Diadaptasi dari \citetitb{Gu2023Mamba}.}
  \label{fig:bab2_mamba_ssm}
\end{figure}

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth]{figures/bab2/fig_slstm_block.png}
  \caption{Blok \emph{scalar LSTM} (sLSTM) dalam arsitektur xLSTM
           \citetitb{Beck2024xLSTM}: \emph{exponential gating} ($i_t = \exp(\cdot)$,
           $f_t = \exp(\cdot)$) menggantikan saturasi sigmoid pada LSTM klasik,
           memberikan kapasitas memori yang lebih besar.
           Diadaptasi dari \citetitb{Beck2024xLSTM}.}
  \label{fig:bab2_slstm}
\end{figure}

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth]{figures/bab2/fig_mlstm_block.png}
  \caption{Blok \emph{matrix LSTM} (mLSTM) dalam arsitektur xLSTM
           \citetitb{Beck2024xLSTM}: memori matriks $C_t \in \mathbb{R}^{d \times d}$
           menyimpan asosiasi \emph{key-value} dengan kapasitas jauh lebih besar
           dibanding memori vektor LSTM klasik.
           Diadaptasi dari \citetitb{Beck2024xLSTM}.}
  \label{fig:bab2_mlstm}
\end{figure}
{\color{blue}Di lini interpretabilitas berbasis arsitektur, kerangka N-BEATS
...
{\color{blue}ketiga keunggulan tersebut --- efisiensi Mamba, kapasitas memori
...
{\color{blue}\textbf{Kuantifikasi ketidakpastian (\emph{uncertainty quantification}, UQ).}
...
Perluasan ke UQ prediksi RUL ditetapkan sebagai \emph{future work} pada Subbab~VI.5.}
...
Metode XAI yang dibahas pada Subbab~\ref{sec:bab2_xai} beroperasi di tingkat
...
Gambar~\ref{fig:bab2_sae_architecture} mengilustrasikan konsep arsitektur
Top-$k$ SAE menurut \citetitb{Cunningham2023TopkSAE}; dalam penelitian ini,
aktivasi diambil dari \emph{hidden state} \emph{backbone} RUL, bukan dari
model bahasa.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.88\textwidth]{figures/bab2/fig_sae_architecture.png}
  \caption{Konsep arsitektur Top-$k$ \emph{Sparse Autoencoder} (SAE)
           \citetitb{Cunningham2023TopkSAE}: aktivasi lapisan (\emph{Activation})
           dari model sumber dikodekan melalui \emph{Encoder} menjadi kode
           \emph{sparse} (\emph{Sparse Coefficient}), lalu didekode kembali
           melalui \emph{Decoder} (\emph{dictionary}) menjadi aktivasi
           rekonstruksi (\emph{Reconstructed Activation}). Hanya sejumlah kecil
           fitur aktif per sampel (Top-$k$); fitur tersebut dipetakan ke
           interpretasi mekanistik (\emph{feature\_id: interpretations}).
           Dalam penelitian ini, model sumber digantikan oleh \emph{backbone}
           RUL prognostik dengan \emph{hidden state}
           $\mathbf{h} \in \mathbb{R}^d$, kamus laten
           $d_{\rm lat} = 8d$, dan $k = 51$ ($\approx 5\%$).
           Diadaptasi dari \citetitb{Cunningham2023TopkSAE}.}
  \label{fig:bab2_sae_architecture}
\end{figure}
...
% -----------------------------------------------------------------
% Subbab~II.7 BARU — tinjauan XAI lintas domain sinyal 1-D
% -----------------------------------------------------------------
{\color{blue}\section{XAI pada Domain Sinyal Satu Dimensi: Tinjauan Lintas Domain}
\label{sec:bab2_xai_lintas_domain}}

{\color{blue}Penerapan XAI pada sinyal satu dimensi tidak terbatas pada domain vibrasi mesin.
...
{\color{blue}Pada domain \emph{electrocardiogram} (ECG), SHAP dan \emph{Integrated Gradients}
...
{\color{blue}Kesamaan lintas domain ini menegaskan bahwa pendekatan XAI berbasis atribusi
...
{\color{blue}Berdasarkan tinjauan pada Subbab~\ref{sec:bab2_pdm_industri}
hingga Subbab~\ref{sec:bab2_xai_lintas_domain}, bagian ini memetakan
...
ketiga rumusan masalah pada Subbab~\ref{sec:bab1_rumusan}.}

{\color{blue}Tabel~\ref{tab:bab2_sota_pdm} menyajikan pemaparan detail
...
\caption{{\color{blue}Pemaparan \emph{state of the art} (SotA) penelitian
...
\multicolumn{4}{@{}l}{\textbf{{\color{blue}Grup A. Diagnostik
...
\midrule
\multicolumn{4}{@{}l}{\textbf{{\color{blue}Grup B. \emph{Explainable AI}
...
\midrule
\multicolumn{4}{@{}l}{\textbf{{\color{blue}Grup C. Prognostik
...
\midrule
\multicolumn{4}{@{}l}{\textbf{{\color{blue}Grup D. Interpretabilitas
...
\midrule
\multicolumn{4}{@{}l}{\textbf{{\color{blue}Grup E. Arsitektur
...
\midrule
\multicolumn{4}{@{}l}{\textbf{{\color{blue}Grup F. Karya internal
disertasi ini}}} \\
\midrule
...
\emph{Hit-rate} BPFI 2,3\,\% (PHM2012, $p < 0{,}001$), BSF
1,6\,\% (XJTU-SY, $p < 0{,}001$); konsistensi urutan dominansi BPFx
...
{\color{blue}Berdasarkan Tabel~\ref{tab:bab2_sota_pdm}, dapat
...
\begin{table}[ht]
  ...
  \caption{Posisi empat karya penelitian ini relatif terhadap literatur umum
           pada {\color{blue}enam} dimensi kritis. Simbol \checkmark = terpenuhi,
  ...
  \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{3.0cm} *{5}{>{\raggedright\arraybackslash}X}@{}}
    ...
    {\color{blue}Validasi industri (data real)}
      & {\color{blue}$\times$} & {\color{blue}$\times$}
      & {\color{blue}$\times$}
      & {\color{blue}\emph{sebagian} (SKF)}
      & {\color{blue}Sangat terbatas} \\[3pt]
...
{\color{blue}empat} kesenjangan yang
...
Kesenjangan ini dijawab oleh {\color{blue}arsitektur multi-tier pada Bab~VI
(RM-3)}.

{\color{blue}\textbf{Kesenjangan~4 (Validasi industri real).} Sebagian
...
{\color{blue}Keempat} kesenjangan tersebut terhubung langsung dengan
{\color{blue}tiga} pertanyaan penelitian
pada Subbab~\ref{sec:bab1_rumusan}: {\color{blue}Kesenjangan~1 dan~2
...
{\color{blue}keempat} kesenjangan disajikan pada Bab~III, dengan detail
...
%     ada di Bab II) -> kini disitir di Subbab~II.5.2 (biru).
...
%      -> 8 bola/15,17deg/1.750rpm/104,6;157,9;68,9;13,1  (sesuai Subbab~IV.11)
...
%   - Permutasi B=1.000: SUDAH selaras di Pendahuluan (Bab~I, B=1.000), Bab V,

...
Subbab~\ref{sec:bab3_bantalan_bpfx} merangkum teori bantalan gelinding dan
...
\noindent di mana $T$ adalah total jumlah rekaman dalam siklus hidup bantalan.
Skema ini sederhana namun memiliki kelemahan karena memaksa model belajar
degradasi yang terjadi sejak awal masa pakai, padahal secara fisik bantalan
...
\emph{Health indicator} (HI) merupakan representasi ringkas dari kondisi bantalan
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}cll@{}}
    ...
    \midrule
    $T$              & skalar          & Total rekaman dalam satu siklus hidup bantalan \\
...
% -----------------------------------------------------------------
\section{Bantalan Gelinding dan Frekuensi Karakteristik}
\label{sec:bab3_bantalan_bpfx}
% -----------------------------------------------------------------

Bantalan gelinding (juga disebut \emph{rolling element bearing}) merupakan
komponen mekanik yang memungkinkan rotasi poros dengan gesekan minimal. Pada
bantalan tipe bola alur dalam (\emph{deep groove ball bearing}), terdapat
empat komponen utama: cincin luar (\emph{outer race}), cincin dalam
(\emph{inner race}), elemen gelinding (\emph{rolling elements}), dan sangkar
(\emph{cage}). Kegagalan bantalan umumnya dimulai sebagai spalling pada salah
...
penampang lintang bantalan bola alur dalam dengan parameter geometri yang
...
\begin{figure}[ht]
  ...
  \end{tikzpicture}
  \caption{Penampang lintang bantalan bola alur dalam (\emph{deep groove ball
           bearing}) dengan anotasi parameter geometri: $D$ = diameter lingkaran
           \emph{pitch}, $d$ = diameter elemen gelinding, $n$ = jumlah elemen
           gelinding. Nilai $D$, $d$, $n$, dan sudut kontak $\theta$ digunakan
...
Saat terjadi spalling pada komponen bantalan, impuls periodik yang ditimbulkan
memiliki frekuensi yang dapat diprediksi dari geometri bantalan dan kecepatan
rotasi poros. Empat frekuensi karakteristik utama didefinisikan sebagai
berikut \citetitb{Lei2018Machinery}.

Misalkan $f_r$ = frekuensi rotasi poros (Hz), $n$ = jumlah elemen gelinding,
$d$ = diameter elemen gelinding (mm), $D$ = diameter lingkaran \emph{pitch}
...
Frekuensi putar elemen gelinding (\emph{Ball Spin Frequency}, BSF) adalah:
...
Sinyal getaran bantalan merupakan superposis dari komponen yang berasal dari
proses fisika yang berbeda: rotasi poros, meshing roda gigi (bila ada), resonansi
struktural, dan impuls periodik akibat cacat bantalan. Ekstraksi fitur yang
...
terhadap perkembangan cacat bantalan.
...
kegagalan akhir bantalan --- tepat kondisi ketika model harus merespons dengan
...
secara khusus relevan untuk fase akhir degradasi bantalan di mana amplitudo
...
banyak $\mathcal{O}(d)$ entri independen. Dalam konteks prediksi RUL bantalan,
...
waktu, seperti sinyal degradasi bantalan yang bersifat non-stasioner.
...
bantalan, fitur pada jendela awal mengandung informasi tentang kondisi dasar
...
\begin{equation}\label{eq:bab3_ig}
  \text{IG}_i(\mathbf{x}) = (x_i - x'_i) \cdot
    \int_0^1 \frac{\partial f(\mathbf{x}' + \alpha(\mathbf{x} - \mathbf{x}'))}
    {\partial x_i} \, d\alpha,
\end{equation}
...
aktualnya) dihukum lebih berat karena dapat menyebabkan operasi bantalan
